...
def insertEvents(events, service, config):
  ...
  for event in main_events:
    ...
    date = temp.timestamp() + (int(event['Lịch học']['Thứ']) - 1 - temp.isocalendar()[2]) * 86400

    this_week = datetime.datetime.now().isocalendar()[1]
    while(this_week + week > datetime.datetime.fromtimestamp(date).isocalendar()[1]): date = date + 604800
